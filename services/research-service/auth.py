"""
JWT validation for research-service (Better Auth JWKS).
Mirrors ai-router/auth.py but uses ResearchSettings.
"""

from __future__ import annotations

import logging
from typing import Optional

import jwt
from fastapi import Depends, HTTPException, Request
from jwt import PyJWKClient

from config import ResearchSettings, get_research_settings

log = logging.getLogger("manthana.research.auth")

_jwks_client: Optional[PyJWKClient] = None


def _get_jwks_client(settings: ResearchSettings) -> PyJWKClient:
    global _jwks_client
    if _jwks_client is None:
        jwks_url = f"{settings.BETTER_AUTH_URL.rstrip('/')}/api/auth/jwks"
        _jwks_client = PyJWKClient(jwks_url, cache_keys=True)
    return _jwks_client


def _extract_bearer_token(request: Request) -> Optional[str]:
    auth = request.headers.get("Authorization")
    if not auth or not auth.lower().startswith("bearer "):
        return None
    return auth[7:].strip()


async def get_current_user_optional(
    request: Request,
    settings: ResearchSettings = Depends(get_research_settings),
) -> Optional[dict]:
    token = _extract_bearer_token(request)
    if not token:
        return None
    try:
        client = _get_jwks_client(settings)
        signing_key = client.get_signing_key_from_jwt(token)
        payload = jwt.decode(
            token,
            signing_key.key,
            algorithms=["EdDSA"],
            issuer=settings.BETTER_AUTH_URL.rstrip("/"),
            audience=settings.BETTER_AUTH_URL.rstrip("/"),
        )
        return payload
    except jwt.ExpiredSignatureError:
        log.debug("JWT expired")
        return None
    except jwt.InvalidTokenError as e:
        log.debug("Invalid JWT: %s", e)
        return None


async def get_current_user(
    request: Request,
    settings: ResearchSettings = Depends(get_research_settings),
) -> dict:
    user = await get_current_user_optional(request, settings)
    if user is None:
        raise HTTPException(
            status_code=401,
            detail="Authentication required",
            headers={"WWW-Authenticate": "Bearer"},
        )
    return user


async def get_protected_user(
    request: Request,
    settings: ResearchSettings = Depends(get_research_settings),
):
    if settings.REQUIRE_AUTH:
        return await get_current_user(request, settings)
    return await get_current_user_optional(request, settings)


async def require_staff_user(
    request: Request,
    settings: ResearchSettings = Depends(get_research_settings),
) -> dict:
    """JWT required; role must be staff/admin (for monitoring endpoints)."""
    user = await get_current_user(request, settings)
    role = str(user.get("role") or user.get("userRole") or "").lower()
    if role in ("staff", "admin", "superadmin"):
        return user
    raise HTTPException(status_code=403, detail="Staff access required")
